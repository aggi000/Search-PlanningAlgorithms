...
def minimax(board,player,ply):
    return minmax(board,player,ply,1)

def minmax(board, player, ply, nodes_expanded):
    ...
    if player=="X":
        ...
        for move in board.available_moves():
            board.perform_move(move,player)#performs move
            nodes_expanded += 1
            y, var, nodes_expanded = minmax(board, other_player,ply-1,nodes_expanded)#recursion
            new_score = y
            if new_score > value:#maximizing
                value = new_score
                column = move
            board.undo_move(move)#board returns to initial state
        ...
        for move in board.available_moves():
            board.perform_move(move,player)#performs move
            nodes_expanded += 1
            z, var,nodes_expanded=minmax(board, other_player, ply - 1,nodes_expanded)#recursion
            new_score = z
            if new_score < value:#minimizing
                value = new_score
                column = move
